...
public class DNxHR extends Shutter {
		...
		Thread thread = new Thread(new Runnable(){			
			...
			public void run() {
				...
				try {
		            ...
		            //Flags
		    		String flags = setFlags(fichier, extension);
					...
					if (Settings.btnExtension.isSelected())
						...
						nomExtension =  "_DNxHR_" + comboFilter.getSelectedItem().toString().replace(" ","_");
		            
		            //Sortie
					String sortieFichier;
					if (caseCreateOPATOM.isSelected())
						 sortieFichier =  sortie.replace("\\", "/") + "/" + fichier.replace(extension, nomExtension + ".mxf");
					else
						 sortieFichier =  sortie.replace("\\", "/") + "/" + fichier.replace(extension, nomExtension + ".mov");
					...
					if (cancelled == false && FFMPEG.error == false)
					{
						...
						String cmd = opatom + frameRate + filterComplex + " -vcodec dnxhd -profile:v " + debit + colorspace + timecode + flags + metadatas + " -y ";
						...
						{						
							FFMPEG.toFFPLAY(loop + FFMPEG.inPoint + sequence + concat + " -i " + '"' + file.toString() + '"' + logo + subtitles + FFMPEG.outPoint + cmd + " -f mxf pipe:play |");
							break;
						}
						...
						if (caseCreateOPATOM.isSelected() && lblOPATOM.getText().equals("OP-Atom") && FFMPEG.saveCode == false && cancelled == false && FFMPEG.error == false)
						{
	...
	protected static String setBitrate() {
		if (comboFilter.getSelectedItem().toString().equals("HQX"))
			return "dnxhr_" + comboFilter.getSelectedItem().toString().toLowerCase() + " -pix_fmt yuv422p10";
		else if (comboFilter.getSelectedItem().toString().equals("444"))
			return "dnxhr_" + comboFilter.getSelectedItem().toString().toLowerCase() + " -pix_fmt yuv444p10";
		else
			return "dnxhr_" + comboFilter.getSelectedItem().toString().toLowerCase() + " -pix_fmt yuv422p";
	}
	...
	protected static String setFlags(String fichier, String extension) { 
		...
		if (caseCreateOPATOM.isSelected() && lblOPATOM.getText().equals("OP-Atom"))
			flags += " -metadata material_package_name=" + '"' + fichier + '"' + " -f mxf_opatom";
					...
					{			
						...
						if (file.getName().contains(file.getParentFile().getName()))
						{
							if (file.getParentFile().getParent().toString().equals(OPAtomFolder.toString()) == false) //Si le fichier est dans un dossier différent du précédent
							{
								...
								do	{									
									newFolder = new File(new File(sortie).getParent() + "/" + n);
									n++;
