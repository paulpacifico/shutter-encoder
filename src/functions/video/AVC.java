...
public class AVC extends Shutter {
			...
			public void run() {
				...
				for (int i = 0 ; i < liste.getSize() ; i++)
				{
					...
					if (caseAS10.isSelected() && FFMPEG.saveCode == false && cancelled == false && FFMPEG.error == false)
					{
						...
						BMXTRANSWRAP.run("-t as11op1a -p -o " + '"' + sortie + "/" + fichier.replace(extension, "_AS11" + comboFilter.getSelectedItem().toString()) + '"' + " " + '"' + fileOut.toString() + '"');
	...
	private static boolean actionsDeFin(String fichier, File fileOut, String sortie) {
		...
		if (caseAS10.isSelected())
			Wetransfer.addFile(new File(sortie + "/" + fichier.replace(fichier.substring(fichier.lastIndexOf(".")), "_AS11" + comboFilter.getSelectedItem().toString())));
		else
