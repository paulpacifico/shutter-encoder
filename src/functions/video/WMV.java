...
public class WMV extends Shutter {
		...
		Thread thread = new Thread(new Runnable(){			
			...
			public void run() {
				...
				try {
					...
					String sortieFichier =  sortie.replace("\\", "/") + "/" + fichier.replace(extension, nomExtension + ".wmv"); 
					...
					if(fileOut.exists())		
					{						
						fileOut = Utils.fileReplacement(sortie, fichier, extension, nomExtension + "_", ".wmv");
						if (fileOut == null)
							continue;						
					}					
					...
					cmd = frameRate + resolution + colorspace + pass + filterComplex + " -vcodec wmv2" + preset + bitrate + gamma + flags + metadatas + " -y ";
