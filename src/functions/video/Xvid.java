...
public class Xvid extends Shutter {
		...
		Thread thread = new Thread(new Runnable(){			
			...
			public void run() {
				...
				try {
					...
					String sortieFichier =  sortie.replace("\\", "/") + "/" + fichier.replace(extension, nomExtension + ".avi"); 
					...
					if(fileOut.exists())		
					{						
						fileOut = Utils.fileReplacement(sortie, fichier, extension, nomExtension + "_", ".avi");
						if (fileOut == null)
							continue;						
					}					
					...
					cmd = frameRate + resolution + colorspace + pass + filterComplex + " -vcodec libxvid" + preset + bitrate + gamma + flags + metadatas + " -y ";
