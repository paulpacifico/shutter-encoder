...
public class OGV extends Shutter {
		...
		Thread thread = new Thread(new Runnable(){			
			...
			public void run() {
				...
				try {
					...
					String sortieFichier =  sortie.replace("\\", "/") + "/" + fichier.replace(extension, nomExtension + ".ogv"); 
					...
					if(fileOut.exists())		
					{						
						fileOut = Utils.fileReplacement(sortie, fichier, extension, nomExtension + "_", ".ogv");
						if (fileOut == null)
							continue;						
					}					
					...
					cmd = frameRate + resolution + colorspace + pass + filterComplex + " -vcodec libtheora" + bitrate + gamma + flags + metadatas + " -y ";
