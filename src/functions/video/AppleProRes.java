...
public class AppleProRes extends Shutter {
			...
			public void run() {
				...
				try {
					...
					if (Settings.btnExtension.isSelected())
						...
						nomExtension =  "_ProRes_" + comboFilter.getSelectedItem().toString().replace(" ","_");
					...
					if (cancelled == false && FFMPEG.error == false)
					{
						...
						if (caseCreateOPATOM.isSelected() && FFMPEG.saveCode == false && cancelled == false && FFMPEG.error == false)
						{
	...
	protected static String setFlags() { 
		...
		if (caseDetails.isSelected())
    	{
			...
			if (value > 0)
				return " -sws_flags lanczos" + metadata;
    	}
		return metadata;
	}
