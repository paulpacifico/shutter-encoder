...
public class XAVC extends Shutter {
			...
			public void run() {
				...
				for (int i = 0 ; i < liste.getSize() ; i++)
				{
					...
					{						
						...
						if (encode)
							FFMPEG.run(" -guess_layout_max 0 " + RecordInputDevice.setInputDevices() + logo + cmd + output.replace("Capture.current", timeStamp).replace("Capture.input", timeStamp));	
						else
						{
							FFMPEG.toFFPLAY(" -guess_layout_max 0 " + RecordInputDevice.setInputDevices() + logo + cmd + " -f matroska pipe:play |");							
							break;
						}						
						...
						FFMPEG.run(loop + FFMPEG.inPoint + sequence + concat + " -guess_layout_max 0 -i " + '"' + file.toString() + '"' + logo + subtitles + FFMPEG.outPoint + cmd + output);	
					else //Preview
					{						
						FFMPEG.toFFPLAY(loop + FFMPEG.inPoint + sequence + concat + " -guess_layout_max 0 -i " + '"' + file.toString() + '"' + logo + subtitles + FFMPEG.outPoint + cmd + " -f mxf pipe:play |");
						break;
					}
