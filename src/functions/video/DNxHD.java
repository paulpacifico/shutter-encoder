...
public class DNxHD extends Shutter {
			...
			public void run() {
				...
				try {
					...
					if (Settings.btnExtension.isSelected())
						...
						nomExtension =  "_DNxHD_" + comboFilter.getSelectedItem().toString().replace(" ","_");
	...
	protected static String setAudioFiles(String audioFiles, File file) {
		...
		for (int i = 0 ; i < liste.getSize() ; i++)
		{
			File audioName = new File(liste.getElementAt(i));
	...
	protected static String setInterlace() {
        ...
        {               
            //Entrelacement uniquement pour le 50i          	 
	...
	protected static String setBitrate() {
		if (comboFilter.getSelectedItem().toString().contains("X"))
			return " " + comboFilter.getSelectedItem().toString().replace(" X", "") + "M -pix_fmt yuv422p10";
		else
			return " " + comboFilter.getSelectedItem().toString() + "M -pix_fmt yuv422p";
	}
	...
	protected static String setDeinterlace() {
		...
		{						
			//désentrelacement automatique pour les formats en progressif
            switch (comboFilter.getSelectedItem().toString())
            {
            	case "36":
            	case "75":
	...
	private static boolean actionsDeFin(String fichier, File fileOut, String sortie, File vidstab) {
		...
		//Erreurs
		if (FFMPEG.error || fileOut.length() == 0 || (caseCreateOPATOM.isSelected() && lblOPATOM.getText().equals("OP-Atom")))
		{
			if (FFMPEG.error)
			{
				FFMPEG.errorList.append(fichier);
