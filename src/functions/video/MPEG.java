...
public class MPEG extends Shutter {
		...
		Thread thread = new Thread(new Runnable(){			
			...
			public void run() {
				...
				try {
					...
					String sortieFichier =  sortie.replace("\\", "/") + "/" + fichier.replace(extension, nomExtension + ".mpg"); 
					...
					if(fileOut.exists())		
					{						
						fileOut = Utils.fileReplacement(sortie, fichier, extension, nomExtension + "_", ".mpg");
						if (fileOut == null)
							continue;						
					}					
					...
					cmd = frameRate + resolution + colorspace + pass + filterComplex + " -vcodec" + codec + preset + bitrate + interlace + gamma + flags + metadatas + " -y ";
	...
	protected static String setDeinterlace() {		
		...
		else if (caseForcerDesentrelacement.isSelected() || (FFPROBE.entrelaced.equals("1") && comboFilter.getSelectedIndex() == 0))
		{
			int doubler = 0;
	...
	protected static String setInterlace() {
        ...
        {               
            if (FFPROBE.entrelaced.equals("1") && FFPROBE.fieldOrder.equals("1")) //Invertion de trames
