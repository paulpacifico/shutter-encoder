...
public class AAC extends Shutter {
			...
			public void run() {
				...
				try {
					...
					if (caseMixAudio.isSelected())						
						sortieFichier =  sortie + "/" + fichier.replace(extension, nomExtension + ".m4a");
					else
						sortieFichier =  sortie + "/" + fichier.replace(extension, ".m4a");
					...
					{						
						if (caseMixAudio.isSelected())						
							fileOut = Utils.fileReplacement(sortie, fichier, extension, nomExtension + "_", ".m4a");
						else
							fileOut = Utils.fileReplacement(sortie, fichier, extension, "_", ".m4a");
						if (fileOut == null)
							continue;	
					}	
					...
					if (caseSplitAudio.isSelected()) //Permet de créer la boucle de chaque canal audio
					{
						if (FFPROBE.surround)
						{
							...
							{								
								...
								+ " -map " + '"' + "[LFE]" + '"' + " " + '"'  + fileOut.toString().replace(".m4a", "_LFE.m4a") + '"'
								+ " -map " + '"' + "[BL]" + '"' + " " + '"'  + fileOut.toString().replace(".m4a", "_BL.m4a") + '"'
								+ " -map " + '"' + "[BR]" + '"' + " " + '"'  + fileOut.toString().replace(".m4a", "_BR.m4a") + '"');
							}
	...
	private static String setAudio(String audio) {
		...
		//Bitrate
		audio += "-acodec aac -b:a "+ comboFilter.getSelectedItem().toString() + "k ";								
	...
	private static void splitAudio(String fichier, String extension, File file, String sortie) throws InterruptedException {
		...
		{			
			...
			for (int i = 1 ; i < 3; i ++)
			{
				//Si le fichier existe
				String yesno = " -y ";
				File fileOut = new File(sortie + "/" + fichier.replace(extension, "_Audio_" + i + ".m4a"));
				if(fileOut.exists())
				{										
					fileOut = Utils.fileReplacement(sortie, fichier, extension, "_Audio_" + i + "_", ".m4a");
					if (fileOut == null)
						yesno = " -n ";	
				}
		...
		else if (FFPROBE.channels > 1 && lblSplit.getText().equals(Shutter.language.getProperty("stereo")))
		{
			...
			for (int i = 1 ; i < FFPROBE.channels + 1; i +=2)
			{
				...
				//Si le fichier existe
				String yesno = " -y ";
				File fileOut = new File(sortie + "/" + fichier.replace(extension, "_Audio_" + number + ".m4a"));
				if(fileOut.exists())
				{										
					fileOut = Utils.fileReplacement(sortie, fichier, extension, "_Audio_" + number + "_", ".m4a");
					if (fileOut == null)
						yesno = " -n ";	
				}
