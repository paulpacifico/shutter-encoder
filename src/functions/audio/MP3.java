...
public class MP3 extends Shutter {
			...
			public void run() {				
				...
				try {						
					...
					if (caseMixAudio.isSelected())						
						sortieFichier =  sortie + "/" + fichier.replace(extension, nomExtension + ".mp3");
					else
						sortieFichier =  sortie + "/" + fichier.replace(extension, ".mp3");	
					...
					{						
						if (caseMixAudio.isSelected())						
							fileOut = Utils.fileReplacement(sortie, fichier, extension, nomExtension + "_", ".mp3");
						else
							fileOut = Utils.fileReplacement(sortie, fichier, extension, "_", ".mp3");
						if (fileOut == null)
							continue;	
					}
					...
					if (caseSplitAudio.isSelected()) //Permet de créer la boucle de chaque canal audio
					{
						if (FFPROBE.surround)
						{
							...
							{								
								...
								+ " -map " + '"' + "[FL]" + '"' + " " + '"'  + fileOut.toString().replace(".mp3", "_FL.mp3") + '"'
								+ " -map " + '"' + "[FR]" + '"' + " " + '"'  + fileOut.toString().replace(".mp3", "_FR.mp3") + '"'
								+ " -map " + '"' + "[FC]" + '"' + " " + '"'  + fileOut.toString().replace(".mp3", "_FC.mp3") + '"'
								+ " -map " + '"' + "[LFE]" + '"' + " " + '"'  + fileOut.toString().replace(".mp3", "_LFE.mp3") + '"'
								+ " -map " + '"' + "[BL]" + '"' + " " + '"'  + fileOut.toString().replace(".mp3", "_BL.mp3") + '"'
								+ " -map " + '"' + "[BR]" + '"' + " " + '"'  + fileOut.toString().replace(".mp3", "_BR.mp3") + '"');
							}
	...
	private static String setAudio(String audio) {
		...
		//Bitrate
		audio += "-acodec mp3 -b:a "+ comboFilter.getSelectedItem().toString() + "k ";								
	...
	private static void splitAudio(String fichier, String extension, File file, String sortie) throws InterruptedException {
		...
		{			
			...
			for (int i = 1 ; i < 3; i ++)
			{
				//Si le fichier existe
				String yesno = " -y ";
				File fileOut = new File(sortie + "/" + fichier.replace(extension, "_Audio_" + i + ".mp3"));
				if(fileOut.exists())
				{										
					fileOut = Utils.fileReplacement(sortie, fichier, extension, "_Audio_" + i + "_", ".mp3");
					if (fileOut == null)
						yesno = " -n ";	
				}
				
				String cmd = " -filter_complex " + '"' + "[a:0]pan=1c|c0=c" + (i - 1) + audioFilter + "[a" + (i - 1) + "]" + '"' + " -map " + '"'+ "[a" + (i - 1) + "]" + '"' + " -acodec mp3 -b:a "+ comboFilter.getSelectedItem().toString() + "k -vn" + yesno;
				FFMPEG.run(FFMPEG.inPoint + " -i " + '"' + file.toString() + '"' + FFMPEG.postInPoint + FFMPEG.outPoint + cmd + '"'  + fileOut + '"');	
		...
		else if (FFPROBE.channels > 1 && lblSplit.getText().equals(language.getProperty("mono")))
		{
			for (int i = 1 ; i < FFPROBE.channels + 1; i ++)
			{
				...
				String cmd = audioFilter + " -map a:" + (i - 1) + " -acodec mp3 -b:a "+ comboFilter.getSelectedItem().toString() + "k -vn" + yesno;
				FFMPEG.run(FFMPEG.inPoint + " -i " + '"' + file.toString() + '"' + FFMPEG.postInPoint + FFMPEG.outPoint + cmd + '"'  + fileOut + '"');	
		...
		else if (FFPROBE.channels > 1 && lblSplit.getText().equals(Shutter.language.getProperty("stereo")))
		{
			...
			for (int i = 1 ; i < FFPROBE.channels + 1; i +=2)
			{
				...
				//Si le fichier existe
				String yesno = " -y ";
				File fileOut = new File(sortie + "/" + fichier.replace(extension, "_Audio_" + number + ".mp3"));
				if(fileOut.exists())
				{										
					fileOut = Utils.fileReplacement(sortie, fichier, extension, "_Audio_" + number + "_", ".mp3");
					if (fileOut == null)
						yesno = " -n ";	
				}
				
				String cmd = " -filter_complex " + '"' + "[0:a:" + (i - 1) + "][0:a:" + i + "]amerge=inputs=2" + audioFilter + "[a]" + '"' + " -map " + '"' + "[a]" + '"' + " -acodec mp3 -b:a "+ comboFilter.getSelectedItem().toString() + "k -vn" + yesno;
				FFMPEG.run(FFMPEG.inPoint + " -i " + '"' + file.toString() + '"' + FFMPEG.postInPoint + FFMPEG.outPoint + cmd + '"'  + fileOut + '"');	
