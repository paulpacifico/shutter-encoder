...
public class AIFF extends Shutter {
				...
				for (int i = 0 ; i < liste.getSize() ; i++)
				{
		           ...
		           //SCANNING
					...
					if (caseMixAudio.isSelected())						
						sortieFichier =  sortie + "/" + fichier.replace(extension, nomExtension + ".aif");
					else
						sortieFichier =  sortie + "/" + fichier.replace(extension, ".aif");
					...
					{						
						if (caseMixAudio.isSelected())						
							fileOut = Utils.fileReplacement(sortie, fichier, extension, nomExtension + "_", ".aif");
						else
							fileOut = Utils.fileReplacement(sortie, fichier, extension, "_", ".aif");
						if (fileOut == null)
							continue;	
					}
	...
	private static void splitAudio(String fichier, String extension, File file, String sortie) throws InterruptedException {
		...
		{			
			...
			for (int i = 1 ; i < 3; i ++)
			{
				//Si le fichier existe
				String yesno = " -y ";
				File fileOut = new File(sortie + "/" + fichier.replace(extension, "_Audio_" + i + ".aif"));
				if(fileOut.exists())
				{										
					fileOut = Utils.fileReplacement(sortie, fichier, extension, "_Audio_" + i + "_", ".aif");
					if (fileOut == null)
						yesno = " -n ";	
				}
				
				String cmd = " -filter_complex " + '"' + "[a:0]pan=1c|c0=c" + (i - 1) + audioSpeed + "[a" + (i - 1) + "]" + '"' + " -map " + '"'+ "[a" + (i - 1) + "]" + '"' + " -acodec pcm_s" + comboFilter.getSelectedItem().toString().replace(" Bits", "") + "be -vn" + yesno;
		...
		else if (FFPROBE.channels > 1 && lblSplit.getText().equals("Mono"))
		{
			for (int i = 1 ; i < FFPROBE.channels + 1; i ++)
			{
				...
				String cmd = audioSpeed + " -map a:" + (i - 1) + " -acodec pcm_s" + comboFilter.getSelectedItem().toString().replace(" Bits", "") + "be -vn" + yesno;
				FFMPEG.run(FFMPEG.inPoint + " -i " + '"' + file.toString() + '"' + FFMPEG.postInPoint + FFMPEG.outPoint + cmd + '"'  + fileOut + '"');	
		...
		else if (FFPROBE.channels > 1 && lblSplit.getText().equals(Shutter.language.getProperty("stereo")))
		{
			...
			for (int i = 1 ; i < FFPROBE.channels + 1; i +=2)
			{
				...
				//Si le fichier existe
				String yesno = " -y ";
				File fileOut = new File(sortie + "/" + fichier.replace(extension, "_Audio_" + number + ".aif"));
				if(fileOut.exists())
				{										
					fileOut = Utils.fileReplacement(sortie, fichier, extension, "_Audio_" + number + "_", ".aif");
					if (fileOut == null)
						yesno = " -n ";	
				}
				
				String cmd = " -filter_complex " + '"' + "[0:a:" + (i - 1) + "][0:a:" + i + "]amerge=inputs=2" + audioSpeed + "[a]" + '"' + " -map " + '"' + "[a]" + '"' + " -acodec pcm_s" + comboFilter.getSelectedItem().toString().replace(" Bits", "") + "be -vn" + yesno;
				FFMPEG.run(FFMPEG.inPoint + " -i " + '"' + file.toString() + '"' + FFMPEG.postInPoint + FFMPEG.outPoint + cmd + '"'  + fileOut + '"');	
