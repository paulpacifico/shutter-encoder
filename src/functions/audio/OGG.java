					...
					if (caseSplitAudio.isSelected()) //Permet de créer la boucle de chaque canal audio
					{
						if (FFPROBE.surround)
						{
							...
							{								
								...
								+ " -map " + '"' + "[FL]" + '"' + " " + '"'  + fileOut.toString().replace(".ogg", "_FL.ogg") + '"'
								+ " -map " + '"' + "[FR]" + '"' + " " + '"'  + fileOut.toString().replace(".ogg", "_FR.ogg") + '"'
								+ " -map " + '"' + "[FC]" + '"' + " " + '"'  + fileOut.toString().replace(".ogg", "_FC.ogg") + '"'
								+ " -map " + '"' + "[LFE]" + '"' + " " + '"'  + fileOut.toString().replace(".ogg", "_LFE.ogg") + '"'
								+ " -map " + '"' + "[BL]" + '"' + " " + '"'  + fileOut.toString().replace(".ogg", "_BL.ogg") + '"'
								+ " -map " + '"' + "[BR]" + '"' + " " + '"'  + fileOut.toString().replace(".ogg", "_BR.ogg") + '"');
							}
	...
	private static void splitAudio(String fichier, String extension, File file, String sortie) throws InterruptedException {
		...
		{			
			...
			for (int i = 1 ; i < 3; i ++)
			{
				//Si le fichier existe
				String yesno = " -y ";
				File fileOut = new File(sortie + "/" + fichier.replace(extension, "_Audio_" + i + ".ogg"));
				if(fileOut.exists())
				{										
					fileOut = Utils.fileReplacement(sortie, fichier, extension, "_Audio_" + i + "_", ".ogg");
					if (fileOut == null)
						yesno = " -n ";	
				}
		...
		else if (FFPROBE.channels > 1 && lblSplit.getText().equals(Shutter.language.getProperty("stereo")))
		{
			...
			for (int i = 1 ; i < FFPROBE.channels + 1; i +=2)
			{
				...
				//Si le fichier existe
				String yesno = " -y ";
				File fileOut = new File(sortie + "/" + fichier.replace(extension, "_Audio_" + number + ".ogg"));
				if(fileOut.exists())
				{										
					fileOut = Utils.fileReplacement(sortie, fichier, extension, "_Audio_" + number + "_", ".ogg");
					if (fileOut == null)
						yesno = " -n ";	
				}
