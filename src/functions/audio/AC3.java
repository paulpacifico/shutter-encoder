...
public class AC3 extends Shutter {
			...
			public void run() {				
				...
				try {						
					...
					if (caseMixAudio.isSelected())						
						sortieFichier =  sortie + "/" + fichier.replace(extension, nomExtension + ".ac3");
					else
						sortieFichier =  sortie + "/" + fichier.replace(extension, ".ac3");
					...
					{						
						if (caseMixAudio.isSelected())						
							fileOut = Utils.fileReplacement(sortie, fichier, extension, nomExtension + "_", ".ac3");
						else
							fileOut = Utils.fileReplacement(sortie, fichier, extension, "_", ".ac3");
						if (fileOut == null)
							continue;	
					}
	...
	private static String setAudio(String audio) {
		...
		//Bitrate
		audio += "-acodec ac3 -b:a "+ comboFilter.getSelectedItem().toString() + "k ";								
	...
	private static void splitAudio(String fichier, String extension, File file, String sortie) throws InterruptedException {
		...
		{			
			...
			for (int i = 1 ; i < 3; i ++)
			{
				...
				String cmd = " -filter_complex " + '"' + "[a:0]pan=1c|c0=c" + (i - 1) + audioFilter + "[a" + (i - 1) + "]" + '"' + " -map " + '"'+ "[a" + (i - 1) + "]" + '"' + " -acodec ac3 -b:a "+ comboFilter.getSelectedItem().toString() + "k -vn" + yesno;
				FFMPEG.run(FFMPEG.inPoint + " -i " + '"' + file.toString() + '"' + FFMPEG.postInPoint + FFMPEG.outPoint + cmd + '"'  + fileOut + '"');	
		...
		else if (FFPROBE.channels > 1 && lblSplit.getText().equals(language.getProperty("mono")))
		{
			for (int i = 1 ; i < FFPROBE.channels + 1; i ++)
			{
				...
				String cmd = audioFilter + " -map a:" + (i - 1) + " -acodec ac3 -b:a "+ comboFilter.getSelectedItem().toString() + "k -vn" + yesno;
				FFMPEG.run(FFMPEG.inPoint + " -i " + '"' + file.toString() + '"' + FFMPEG.postInPoint + FFMPEG.outPoint + cmd + '"'  + fileOut + '"');	
		...
		else if (FFPROBE.channels > 1 && lblSplit.getText().equals(Shutter.language.getProperty("stereo")))
		{
			...
			for (int i = 1 ; i < FFPROBE.channels + 1; i +=2)
			{
				...
				String cmd = " -filter_complex " + '"' + "[0:a:" + (i - 1) + "][0:a:" + i + "]amerge=inputs=2" + audioFilter + "[a]" + '"' + " -map " + '"' + "[a]" + '"' + " -acodec ac3 -b:a "+ comboFilter.getSelectedItem().toString() + "k -vn" + yesno;
				FFMPEG.run(FFMPEG.inPoint + " -i " + '"' + file.toString() + '"' + FFMPEG.postInPoint + FFMPEG.outPoint + cmd + '"'  + fileOut + '"');	
