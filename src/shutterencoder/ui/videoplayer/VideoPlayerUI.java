...
public class VideoPlayerUI {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (btnPlay.getName().equals("pause"))
				{
					...
					else
					{
						...
						{
							getTimePoint(VideoPlayerCore.playerCurrentFrame); 
						}
					}
		...
		btnMarkIn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (caseApplyCutToAll.isVisible() == false || caseApplyCutToAll.isSelected() == false)
				{
					...
					waveformContainer.repaint();							
					updateGrpIn(VideoPlayerCore.playerCurrentFrame);
		...
		player = new JPanel() {
		    ...
		    protected void paintComponent(Graphics g) {
		        ...
		        {		
		            ...
		            cursorCurrentFrame.setLocation((int) Math.round((double) (waveformContainer.getWidth() * Timecode.setNTSCtimecode(VideoPlayerCore.playerCurrentFrame)) / slider.getMaximum()), 0);
		        }
	...
	private void sliders() {
		...
		slider.addChangeListener(new ChangeListener(){
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (VideoPlayerCore.playerVideo != null && sliderChange)
				{							
					if (slider.getValue() > 0)
					{							
						VideoPlayerCore.playerSetTime(slider.getValue());						
					}
					else
					{
						VideoPlayerCore.playerSetTime(0);												
		    ...
		    {		
                ...
                if (Shutter.list.getSize() > 0 && Shutter.comboFonctions.getSelectedItem().equals(Shutter.language.getProperty("functionSubtitles")) == false)
                {
	                ...
	                {		                
			            ...
			            double bufferPosition = (VideoPlayerCore.bufferCurrentFrame - VideoPlayerCore.bufferedFrames.size());       
			            
			            int x = (int) ((double) waveformContainer.getSize().width * bufferPosition) / slider.getMaximum();
			            int width = (int) ((double) waveformContainer.getSize().width * VideoPlayerCore.bufferedFrames.size()) / slider.getMaximum();
			...
			public void mouseReleased(MouseEvent e) {	
                ...
                {	
					...
					{			
						...
						cursorHead.setLocation(cursorWaveform.getX() - 5, cursorWaveform.getY());
						updateGrpOut(VideoPlayerCore.playerCurrentFrame);
					}		
	...
	public static void updateGrpIn(double timeIn) {
		...
		caseInH.setText(Shutter.formatter.format(Math.floor(timeIn / fps / 3600)));
		caseInM.setText(Shutter.formatter.format(Math.floor(timeIn / fps / 60) % 60));
		caseInS.setText(Shutter.formatter.format(Math.floor(timeIn / fps) % 60));    		
		caseInF.setText(Shutter.formatter.format(Math.floor(timeIn % fps)));
	...
	public static void getTimePoint(double time) {	
    	...
    	{    	    		
			...
			String h = Shutter.formatter.format(Math.floor(time / fps / 3600));
			String m = Shutter.formatter.format(Math.floor(time / fps / 60) % 60);
			String s = Shutter.formatter.format(Math.floor(time / fps) % 60);   
			String f = Shutter.formatter.format(Math.floor(time % fps));
